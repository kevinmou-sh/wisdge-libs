...
public class CookieManager {
	...
	/**
	 * Retrieves and stores cookies returned by the host on the other side of the the open java.net.URLConnection. The connection MUST have been opened using
	 * the <i>connect()</i> method or a IOException will be thrown.
	 * 
	 * @param conn
		...
		for (int i = 1; (headerName = conn.getHeaderFieldKey(i)) != null; i++) {
			...
			if (headerName.equalsIgnoreCase(SET_COOKIE)) {
				...
				while (st.hasMoreTokens()) {
					String token = st.nextToken();
					cookie.put(token.substring(0, token.indexOf(NAME_VALUE_SEPARATOR)).toLowerCase(), token.substring(token.indexOf(NAME_VALUE_SEPARATOR) + 1,
							token.length()));
				}
			} else {
	...
	/**
	 * Prior to opening a URLConnection, calling this method will set all unexpired cookies that match the path or subpaths for thi underlying URL The
	 * connection MUST NOT have been opened method or an IOException will be thrown.
	 * 
	 * @param conn
