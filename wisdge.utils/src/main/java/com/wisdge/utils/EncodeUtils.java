...
public class EncodeUtils {
	...
	/**
	 * 解码<br>
	 * 说明：本方法保证 不论参数string是否经过escape()编码，均能得到正确的“解码”结果
	 * 
	 * @param string
	 *            编码后的字符串
	 * @return 原始字符串
		...
		while (i < len) {
			...
			if ('A' <= ch && ch <= 'Z') {
				...
				if ('u' != string.charAt(i + 1)) {
					cint = (cint << 4) | val[string.charAt(i + 1)];
					cint = (cint << 4) | val[string.charAt(i + 2)];
					i += 2;
				} else {
					cint = (cint << 4) | val[string.charAt(i + 2)];
					cint = (cint << 4) | val[string.charAt(i + 3)];
					cint = (cint << 4) | val[string.charAt(i + 4)];
					cint = (cint << 4) | val[string.charAt(i + 5)];
					i += 5;
				}
