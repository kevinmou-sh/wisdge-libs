...
public class SM2Util extends GMBaseUtil {
	...
	public static final java.security.spec.ECParameterSpec JDK_EC_SPEC = new java.security.spec.ECParameterSpec(JDK_CURVE, JDK_G_POINT, SM2_ECC_N, SM2_ECC_H.intValue());
	...
	public static String[] generateKeysHex() {
		AsymmetricCipherKeyPair keyPair = generateKeyPairParameter();
		ECPrivateKeyParameters priKey = (ECPrivateKeyParameters) keyPair.getPrivate();
		ECPublicKeyParameters pubKey = (ECPublicKeyParameters) keyPair.getPublic();

		String[] keysHex = new String[3];
		keysHex[0] = ByteUtils.toHexString(priKey.getD().toByteArray()).toUpperCase();
		keysHex[1] = ByteUtils.toHexString(pubKey.getQ().getAffineXCoord().getEncoded()).toUpperCase();
		keysHex[2] = ByteUtils.toHexString(pubKey.getQ().getAffineYCoord().getEncoded()).toUpperCase();
		keysHex[3] = ByteUtils.toHexString(pubKey.getQ().getEncoded(false)).toUpperCase();
		return keysHex;
	}

	public static ECPublicKeyParameters recovePublicKey(String pubHex) {
		return BCECUtil.createECPublicKeyParameters(pubHex, SM2Util.CURVE, SM2Util.DOMAIN_PARAMS);
	}

	public static ECPrivateKeyParameters recovePrivateKey(String priHex) {
	...
	public static String encrypt(String pubHex, String content) throws InvalidCipherTextException {
		ECPublicKeyParameters pubKey = recovePublicKey(pubHex);
		return ByteUtils.toHexString(SM2Util.encrypt(pubKey, content.getBytes()));
	}
	...
	public static String decrypt(String priHex, String encodedContent) throws InvalidCipherTextException {
		...
		return new String(SM2Util.decrypt(priKey, ByteUtils.fromHexString(encodedContent)));
	}
